...
var PruneClusterForLeaflet = ((<any>L).Layer ? (<any>L).Layer : L.Class).extend(
  {
    ...
    BuildLeafletCluster: function (
      ...
      m.on("click", () => {
        ...
        var markersArea: PruneCluster.Marker[] =
          this.Cluster.FindMarkersInArea(cbounds);
        ...
        if (b) {
          ...
          var zoomLevelBefore = this._map.getZoom(),
            zoomLevelAfter = this._map.getBoundsZoom(
              bounds,
              false,
              new L.Point(20, 20)
            );
          ...
          if (zoomLevelAfter === zoomLevelBefore) {
            ...
            if (
              markersArea.length < 200 ||
              zoomLevelAfter >= this._map.getMaxZoom()
            ) {
    ...
    ProcessView: function () {
      ...
      var objectsOnMap: PruneCluster.Cluster[] = this._objectsOnMap,
        ...
        markersOnMap: PruneCluster.LeafletMarker[] = new Array(
          objectsOnMap.length
        );

      // Second step : By default, all the leaflet markers should be removed
      for (var i = 0, l = objectsOnMap.length; i < l; ++i) {
        var marker = (<PruneCluster.ILeafletAdapterData>objectsOnMap[i].data)
          ._leafletMarker;
      ...
      clusterCreationList =
        clusterCreationListPopOne.concat(clusterCreationList);
        ...
        if (idata._leafletMarker._removeFromMap) {
          ...
          if (marker._zoomLevel === zoom) {
            ...
            (latMargin =
              (icluster.bounds.maxLat - icluster.bounds.minLat) * marginRatio),
              (lngMargin =
                (icluster.bounds.maxLng - icluster.bounds.minLng) *
                marginRatio);
    ...
    FitBounds: function (withFiltered: boolean = true) {
      var bounds: PruneCluster.Bounds =
        this.Cluster.ComputeGlobalBounds(withFiltered);
      if (bounds) {
        this._map.fitBounds(
          new L.LatLngBounds(
